...
for code in codes:
    ...
    df.index=df1['end_date'].dt.year
    df.columns=['净资产收益率_加权%','投入资本回报率%','销售毛利率%','销售净利率%','营业总收入增长率%','归母净利润增长率%','总资产周转率',\
                '应收账款周转率','应付账款周转率','资产负债率%','流动比率%','速动比率%','经营性现金净流量/营业总收入%',\
                '经营活动净现金/归属母公司的净利润% ',]
    df=df.applymap(lambda x: '%.0f'%x)

    df3=get_fundamentals(table='trading_derivative_indicator', symbols='SZSE.000776', start_date='1990-01-01', end_date='2023-01-12',
                     fields='PB,PETTM,TCLOSE', limit=40000, df=True)
    df=pd.concat([df,df3.iloc[:,3:]],axis=1)
    df.to_excel(Fr'C:\xyzy\1lhjr\5gsfx\cwsj\{code[5:]}.xlsx')