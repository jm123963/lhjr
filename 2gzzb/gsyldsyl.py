...
try:
    ...
    # 指数 涨跌幅
    data=c.csd("801821.SWI","PCTCHANGE","2000-08-01",""+date+"","period=3,adjustflag=1,curtype=1,order=1,market=CNSESH,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\2clzb\gsyl.xls', encoding='utf-8-sig', index=None)

    data=c.csd("801823.SWI","PCTCHANGE","2000-08-01",""+date+"","period=3,adjustflag=1,curtype=1,order=1,market=CNSESH,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\2clzb\dsyl.xls', encoding='utf-8-sig', index=None)
...
wb = xw.Book(r'C:\xyzy\1lhjr\2clzb\gsyl.xls')
sht = wb.sheets[0]
rng = sht.range('c1').expand('table')
nrows = rng.rows.count
gsyl = sht.range(f'c2:c{nrows}').value
wb.close()

wb = xw.Book(r'C:\xyzy\1lhjr\2clzb\dsyl.xls')
sht = wb.sheets[0]
rng = sht.range('c1').expand('table')
nrows = rng.rows.count
dsyl = sht.range(f'c2:c{nrows}').value
wb.close()

from pandas import DataFrame
l1 = gsyl
l2 = dsyl
df = DataFrame({'gsyl': l1, 'dsyl': l2})
df.to_excel(r'C:\xyzy\1lhjr\2clzb\gsyldsyl.xls', sheet_name='Sheet1', index=False)

import pandas as pd
data_= pd.read_excel(r'C:\xyzy\1lhjr\2clzb\gsyldsyl.xls')
data_['gsylcydsyl'] = round(data_['gsyl'] / data_['dsyl']).astype('str')
data_['gsylcydsyl'].to_excel(r'C:\xyzy\1lhjr\2clzb\gsylcydsyl.xls', index=False)
...
wb = xw.Book(r'C:\xyzy\1lhjr\2clzb\gsylcydsyl.xls')
sht = wb.sheets[0]
...
x1=[]
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\2clzb\gsyldsyl.xls')
table = data.sheets()[0]
...
plt.title('高市盈率指数/低市盈率指数',fontsize=30)
plt.xlabel(u"")
plt.ylabel(u"")
plt.legend()
plt.savefig(r'C:\xyzy\1lhjr\2clzb\gsyldsyl.png',c = 'k')
