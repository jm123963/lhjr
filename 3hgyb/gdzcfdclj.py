...
try:
    ...
    # EMM00027210 固定资产投资完成额:累计同比
    data=c.edb("EMM00027210", "IsLatest=0,StartDate=2019-01-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\gdzclj.xls', encoding='utf-8-sig', index=None)
    
    # EMI00120220 城镇房地产开发投资完成额:累计同比
    data=c.edb("EMI00120220", "IsLatest=0,StartDate=2019-01-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\fdclj.xls', encoding='utf-8-sig', index=None)
...
# gdzclj
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\gdzclj.xls')
table = data.sheets()[0]
...
plt.plot(x1, y1, label = "固定资产投资完成额:累计同比")
plt.xticks(rotation=90)

# fdclj
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\fdclj.xls')
table = data.sheets()[0]
st = data.sheet_by_index(0)
...
plt.plot(x1, y1, label = "城镇房地产开发投资完成额:累计同比")
plt.xticks(rotation=90)

plt.legend(loc='best')
plt.title('固定资产、房地产投资累计同比')
plt.savefig(r'C:\xyzy\1lhjr\3hgyb\gdzcfdclj.png',c = 'k')
plt.show()
print('png end')

# 1—5月份，全国固定资产投资205,964亿元，同比增长6.2%。1—5月份，全国房地产开发投资52,134 亿元，同比下降4.0%；其中，住宅投资39,521亿元，下降3.0%。