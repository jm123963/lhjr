...
try:
    ...
    # EMG00000733 美国:CPI:非季调:当月同比
    data=c.edb("EMG00000733", "IsLatest=0,StartDate=2020-09-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\mgcpi.xls', encoding='utf-8-sig', index=None)
    
    # EMG00000734 美国:核心CPI:非季调:当月同比
    data=c.edb("EMG00000734", "IsLatest=0,StartDate=2020-09-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\mghxcpi.xls', encoding='utf-8-sig', index=None)
...
# mgcpi
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\mgcpi.xls')
table = data.sheets()[0]
...
plt.plot(x1, y1, label = "美国:CPI:非季调:当月同比")
plt.xticks(rotation=90)

# mghxcpi
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\mghxcpi.xls')
table = data.sheets()[0]
st = data.sheet_by_index(0)
...
plt.plot(x1, y1, label = "美国:核心CPI:非季调:当月同比")
plt.xticks(rotation=90)

plt.legend(loc='best')
plt.title('美国CPI 与核心CPI（%）')
plt.savefig(r'C:\xyzy\1lhjr\3hgyb\mgcpi.png',c = 'k')
plt.show()
print('png end')

# 5月美国CPI同比8.6%，前值8.3%。核心CPI同比6.0%，前值6.2%。CPI环比1.0%，前值0.3%，核心CPI环比0.6%，前值0.6%。