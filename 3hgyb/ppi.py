...
try:
    ...
    data=c.edb("EMM00073348", "IsLatest=0,StartDate=2020-07-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\ppitb.xls', encoding='utf-8-sig', index=None)
    
    # EMM00073349 PPI:全部工业品:环比
    data=c.edb("EMM00073349", "IsLatest=0,StartDate=2020-07-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\ppihb.xls', encoding='utf-8-sig', index=None)
...
# ppitb
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\ppitb.xls')
table = data.sheets()[0]
...
plt.plot(x1, y1, label = "PPI:当月同比")
plt.xticks(rotation=90)

# ppihb
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\ppihb.xls')
table = data.sheets()[0]
st = data.sheet_by_index(0)
...
plt.plot(x1, y1, label = "PPI:环比")
plt.xticks(rotation=90)

plt.legend(loc='best')
plt.title('中国PPI 同比及环比(%)')
plt.savefig(r'C:\xyzy\1lhjr\3hgyb\ppi.png',c = 'k')
plt.show()
print('png end')

# 上半年PPI 整体从高位往下回落。5 月PPI 同比上涨6.4%，环比上涨0.1%。涨幅均继续回落。