...
try:
    ...
    data=c.edb("EMM00072301", "IsLatest=0,StartDate=2020-07-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\cpitb.xls', encoding='utf-8-sig', index=None)
    
    # EMM00072343 CPI:环比
    data=c.edb("EMM00072343", "IsLatest=0,StartDate=2020-07-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\cpihb.xls', encoding='utf-8-sig', index=None)
...
# fdcxse
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\cpitb.xls')
table = data.sheets()[0]
...
plt.plot(x1, y1, label = "CPI:当月同比")
plt.xticks(rotation=90)

# fdcxsmj
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\cpihb.xls')
table = data.sheets()[0]
st = data.sheet_by_index(0)
...
plt.plot(x1, y1, label = "CPI:环比")
plt.xticks(rotation=90)

plt.legend(loc='best')
plt.title('中国CPI 同比及环比(%)')
plt.savefig(r'C:\xyzy\1lhjr\3hgyb\cpi.png',c = 'k')
plt.show()
print('png end')

# 通胀保持温和，CPI与上月持平。5月份全国CPI同比上涨2.1%，涨幅与上月相同，环比则由上月上涨0.4%转为下降0.2%。