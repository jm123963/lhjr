...
try:
    ...
    # EMI01005463 全国农产品批发月平均价:猪肉(白条猪)
    data=c.edb("EMI01005463", "IsLatest=0,StartDate=2021-01-01,EndDate="+date+",AdjustFlag=1,Rowindex=none,Ispandas=1")
    data.to_excel(r'C:\xyzy\1lhjr\3hgyb\btz.xls', encoding='utf-8-sig', index=None)
...
# btz
data = xlrd.open_workbook(r'C:\xyzy\1lhjr\3hgyb\btz.xls')
table = data.sheets()[0]
...
plt.legend(loc='best')
plt.title('全国猪肉(白条猪)批发月平均价:')
plt.savefig(r'C:\xyzy\1lhjr\3hgyb\btz.png',c = 'k')
plt.show()
print('png end')

#中央储备猪肉收储工作继续开展，猪肉价格跌幅开始逐步收窄。5月白条肉月均价格同比降幅收在至-23%，不过进入6 月份白条肉价格基本与去年同期持平。